...
def process(item,soup,table_1_res,kd_xqinfo_res,baby_info_res):
...
class A189spSpider(scrapy.Spider):
 ...
 #        print self.verificationErrors
	...
	def parse(self, response):
		...
		if 'broadband' in response.url:
			...
			for body in bodys:
				...
				if kd_xqinfo_res == None and table_1_res == None and baby_info_res == None:
					...
					process(item,soup,table_1_res,kd_xqinfo_res, baby_info_res)
					items.append(item)
					yield item
					break
		...
		# elif 'queryPackageXq' in response.url:
		# 	bodys = response.body.split('<html>')		
		# 	for body in bodys:
		# 		soup = bs(body)
		# 		kd_xqinfo_res = soup.find('div', class_='kd_xqinfo')
		# 		table_1_res = soup.find('table', class_='table_1')
		# 		if kd_xqinfo_res == None and table_1_res == None:
		# 			continue
		# 		else:
		# 			item = CrawlerItem()
		# 			item['url'] = response.url.strip()
		# 			process(item,soup,table_1_res,kd_xqinfo_res)
		# 			items.append(item)
		# 			yield item
		# 			break
		...
		# 		yield Request(queryPackageXqPrefix+sxp+'.html', callback=self.parse)
			...
			url = 'http://js.189.cn/nmall/product/queryPackageList.do'
